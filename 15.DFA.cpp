...
void preprocess(int k, int Table[][2])
{
    ...
    for(int state = 0; state<k; state++)
    {
        ...
        trans1 = (state<k)?trans1: trans1-k;
...
void isDivisibility(int num,  int *state, int Table[][2])
{
    if(num!=0)
    {
        isDivisibility(num>>1, state, Table);
        *state = Table[*state][num&1];
    }
}

int isDivisibility(int num, int k)
{
    ...
    int state = 0;
    isDivisibility(num, &state, Table);
...
int main()
{
    ...
    int remainder = isDivisibility(num, k);
    if(remainder == 0)
        printf("Divisible\n");
    else
